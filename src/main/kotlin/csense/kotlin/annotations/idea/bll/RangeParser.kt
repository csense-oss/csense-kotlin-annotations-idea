...
sealed class RangeParser<T>(
    ...
    object DoubleRangeParser : RangeParser<Double>(
        false,
        setOf("DoubleLimit", "Range"), //range is from jetbrains annotations
        Double.MIN_VALUE,
        Double.MAX_VALUE,
    ...
    object AndroidFloatRange : RangeParser<Double>(
        ...
        Double.MIN_VALUE,
        Double.MAX_VALUE,
        listOf("Double", "Float", "Double?", "Float?"),
        UExpression::asDouble,
        KtExpression::asDouble,
        { from: Double, to: Double, value: Double -> value in from..to },
        { from: Double, to: Double -> from == to },
    ...
    fun List<UAnnotation?>.findThis(): UAnnotation? = find {
        it != null &&
                it.attributeValues.size <= 2 && annotationNames.contains(
    ...
    fun computeErrorMessage(argAnnotations: List<UAnnotation?>, valueArgument: KtValueArgument): String {
        val expression = valueArgument.getArgumentExpression() ?: return "Failed to get argument expression"
    ...
    fun computeErrorMessage(argAnnotations: List<UAnnotation?>, expression: KtExpression): String {
        ...
        val asUExpression = expression.toUElementOfType<UExpression>()
            ?: return ""
